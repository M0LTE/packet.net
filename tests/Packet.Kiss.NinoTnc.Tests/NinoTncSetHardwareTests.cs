...
public class NinoTncSetHardwareTests
{
    ...
    {
        NinoTncSetHardware.BuildPayloadByte(mode, persist).Should().Be(expectedPayload);
    }

    [Fact]
    public void Out_Of_Range_Mode_Throws()
    {
        ((Action)(() => NinoTncSetHardware.BuildPayloadByte(16, persistToFlash: true))).Should().Throw<ArgumentOutOfRangeException>();
        ((Action)(() => NinoTncSetHardware.BuildPayloadByte(255, persistToFlash: false))).Should().Throw<ArgumentOutOfRangeException>();
    }
    ...
    public void Kiss_Frame_Encodes_As_Sethw_Command_With_Mode_Payload()
    {
        ...
        frame.Should().Equal(new byte[] { 0xC0, 0x06, 0x16, 0xC0 });
    }
    ...
    public void Kiss_Frame_Uses_Port_Nibble()
    {
        ...
        frame.Should().Equal(new byte[] { 0xC0, 0x26, 0x06, 0xC0 });
    }
}
