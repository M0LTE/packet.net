...
public sealed class RefreshTokenServiceTests
{
    ...
    [Fact]
    public void A_rotated_old_token_is_rejected_as_reuse_and_burns_the_family()
    {
        var (svc, store, _) = Make();
        var first = svc.Issue("m0lte")!;
        ...
        // Replaying the OLD (already-consumed) token is reuse → revoke the whole family.
    ...
    [Fact]
    public void Reuse_of_a_revoked_token_revokes_a_sibling_in_the_family()
    {
        ...
        // Rotate A once, then replay the consumed A token → family A is burned.
    ...
    private sealed class InMemoryRefreshStore : IRefreshTokenStore
    {
        ...
        public bool Revoke(string tokenHash)
        {
            ...
            rows[tokenHash] = r with { Revoked = true };
            return true;
        }
    ...
    private sealed class FaultingRefreshStore : IRefreshTokenStore
    {
        ...
        public bool Revoke(string tokenHash) => false;
