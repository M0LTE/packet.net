# Publishing `@packet-net/ax25` and `ax25sdl` to npm
...
| Per-package identity | `Packet.Ax25` (PascalCase, dotted) | `@packet-net/ax25` (lowercase, scoped) |
...
| Install a package | `dotnet add package Packet.Ax25` | `npm install @packet-net/ax25` |
...
- **[`web/ax25/`](../../web/ax25/)** → npm name `@packet-net/ax25` (scoped under `@packet-net`). The browser-targeted library — depends on `ax25sdl`.

The order matters: `ax25sdl` must be on npmjs.com before `@packet-net/ax25` can install it by version.
...
Either path makes `@packet-net/ax25` a valid name you can publish.

`ax25sdl` is unscoped — it doesn't need a scope, it just needs to be a unique name on npmjs.com. As of writing, `ax25sdl` is not taken; if someone has grabbed it by the time you publish, prefix it with the `@packet-net` scope (`@packet-net/ax25sdl`) and update `web/ax25/package.json`'s `dependencies` to match.
...
### Step 4 — flip `web/ax25`'s dependency from `file:` to a version

For local development, `web/ax25/package.json` declares its dependency on `ax25sdl` as:
...
> [!IMPORTANT]
> Don't commit this `file:` → `^0.1.0` flip to `main`. It breaks the local-dev flow until the next codegen run rebuilds `ts-spec/dist/`. The GitHub Actions workflow in `.github/workflows/npm-publish.yml` does this flip in CI before publishing, then `web/ax25/package.json` on `main` keeps the `file:` reference.

### Step 5 — build and publish `@packet-net/ax25`
...
`--access public` is required for scoped packages on a free account (scoped packages default to private). Output should end with `+ @packet-net/ax25@0.1.0`.

Verify at <https://www.npmjs.com/package/@packet-net/ax25>.
...
3. **Commit** the version + changelog bump:
   ...
   ```sh
   git add ts-spec/package.json web/ax25/package.json web/ax25/CHANGELOG.md
...
4. Flips `web/ax25`'s `ax25sdl` dependency from `file:` to a registry version (`^<package.json version>`).
5. Installs + builds `web/ax25`.
6. Publishes `@packet-net/ax25` with `npm publish --access public`.
...
2. In `web/ax25/package.json`, change `"dependencies": { "ax25sdl": ... }` to `"@packet-net/ax25sdl": ...`.
3. In `web/ax25/src/**/*.ts`, search-and-replace `from "ax25sdl"` → `from "@packet-net/ax25sdl"`.
4. Republish.

### "ERESOLVE — peer dep" complaints

`@packet-net/ax25` declares `ax25sdl` as a **regular** dependency, not a peer. If you see ERESOLVE during local dev, it usually means `ts-spec/dist/` is out of date — rebuild it:

```sh
cd ts-spec && npm run build
cd ../web/ax25 && rm -rf node_modules && npm install
```
...
You forgot step 6 — `git checkout -- web/ax25/package.json web/ax25/package-lock.json`. Restore the `file:` dependency reference and `npm install` will find `ts-spec/dist/` again.
