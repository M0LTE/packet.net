...
- **V3 — XID parameter negotiation (the MDL)** 🟡 part 1 done (codec); FSM is figure-blocked (packet.net + ax25-ts). Two parts: **(1) the XID info-field codec** ✅ (§4.3.3.7 / Fig 4.5 TLVs — the spec numbers this §4.3.3.7, not §4.3.4.3 as the arc text first guessed: FI 0x82 + GI 0x80 + GL + ordered PI/PL/PV for Classes of Procedures PI=2, HDLC Optional Functions PI=3 incl. modulo + REJ/SREJ + segmenter, I-Field Length Rx PI=6 (bits), Window k Rx PI=8, Ack Timer T1 PI=9, Retries N2 PI=10). Done as `Packet.Ax25.Xid.XidInfoField` (encode/parse) + typed `ClassesOfProcedures` / `HdlcOptionalFunctions` / `XidParameters`, strict-by-default with `XidParseOptions` leniency knobs. See §17. **(2) the MDL/XID negotiation FSM** ⬜ — **figure-blocked**: figc5.1/figc5.2 (Ready/Negotiating states) + figc5.3–5.8 (the per-parameter negotiation subroutines) are **not transcribed** in `m0lte/ax25sdl` (its `spec-sdl/v2.2` has only the six DataLink pages + Subroutines; no MDL pages), so the negotiation FSM **cannot** be driven from the SDL tables the way the data-link FSM is. The data-link side already emits the `MDL-NEGOTIATE-request` poke (`Ax25ActionVerb.MDLNEGOTIATERequest` → `MdlNegotiateRequestSignal`) and the `MdlNegotiate{Request,Confirm}` / `MdlErrorIndicate` / `XidReceived` events exist, but nothing consumes them — the signal is sent into a void. **Two routes for part 2** (Tom's call): (a) transcribe figc5.1–5.8 in ax25sdl (needs Tom's figure input → ax25sdl issue) then drive the FSM from the generated tables; or (b) hand-write a small Ready/Negotiating FSM from the §6.3.2 + App-C5 *prose* (it's a single command/response exchange — small enough to do faithfully from prose, the way the harness drives the figc4.x machine). The negotiation then applies the agreed params and **replaces the forced `SrejEnabled`/`IsExtended`/`K`/`N1` config with negotiated values**; v2.0 fallback (peer FRMRs the XID → v2.0 defaults, §6.3.2 ¶1418 / App-C5.3 ¶2458). *Exit (part 2):* SREJ + segmentation + modulo + window are enabled by an actual XID exchange, not a test flag.
...
| Management Data-Link | C5.1, C5.2 (+ C5.3–C5.8 subroutines) | ~2 (+6 subs) | Ready, Negotiating | ⬜ none — *not transcribed in `m0lte/ax25sdl`* (only the DataLink pages are). The XID *info-field codec* shipped (§5.Z V3 part 1, §17 2026-06-02) but that's the wire TLV payload, **not** this FSM; driving the negotiation needs either figc5.x transcribed upstream or a prose-built FSM. See the §5.Z V3 bullet. |
