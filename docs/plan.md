...
**Validation.** Built under `TreatWarningsAsErrors` (0 errors across `src/`). Full default-filtered suite green — `Packet.NetRom.Tests` 39, `Packet.Node.Tests` 176 (up from 169), every other project unchanged. Interop `NetRomNodesIngestViaNetsim` green locally (`docker compose -f docker/compose.interop.yml up -d --wait` + `dotnet test --filter Category=Interop`), confirmed reliable across repeated runs.
