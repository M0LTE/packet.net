...
- **Slice 3 — REST API + auth. 🟡 contract LOCKED + the UI (Slice-5) built ahead of it; backend pending.** The §7 API is locked as OpenAPI 3.1 in [`docs/node-api.yaml`](node-api.yaml) (derived from the built UI's typed client + the design handoff). The endpoints/SSE/auth shape: minimal-API endpoints (status, ports + lifecycle, sessions + connect/disconnect/send, netrom/routes, config + schema/raw/validate, links, log) + the `/events` SSE feed + Argon2id local users + WebAuthn/passkeys + JWT `read`/`operate`/`admin` scopes; first-start admin bootstrap + one-time `/setup?token=…`; the web server serves the SPA + stops being inert. **The backend implementation is the named next arc** (read+SSE+static-serve+auth first; writes/actions follow — auth + writes are security-sensitive, not rushed). See §17 (2026-06-08 Phase-5-UI entry).
